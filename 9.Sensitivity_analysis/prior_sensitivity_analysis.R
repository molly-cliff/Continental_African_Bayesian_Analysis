...
formula_baseline <- outbreak_2 ~ aod_scale +
  wind_strength_scale +
  humidity_scale +
  wind_dir +
  aod_scale:wind_dir +
...
# ---- 2. Tight variant: more skeptical of large random-effect variance ----
# RW2 threshold halved (0.5 -> 0.25); BYM2 precision threshold halved (1 -> 0.5)
formula_tight <- outbreak_2 ~ aod_scale +
  wind_strength_scale +
  humidity_scale +
  wind_dir +
  aod_scale:wind_dir +
  ...
  f(area, model = "bym2", graph = ken.adj, scale.model = TRUE, constr = TRUE,
    hyper = list(
      ...
      phi  = list(prior = "pc", param = c(0.5, 3/4))
    ))

# ---- 3. Loose variant: more permissive of large random-effect variance ----
# RW2 threshold doubled (0.5 -> 1); BYM2 precision threshold doubled (1 -> 2);
# phi alpha shifted from 2/3 to 1/3 (more weight toward structured spatial variance)
formula_loose <- outbreak_2 ~ aod_scale +
  wind_strength_scale +
  humidity_scale +
  wind_dir +
  aod_scale:wind_dir +
...
# ---- 4. Fit all three models with identical settings otherwise ----
fit_args <- list(
  data = merged_data,
  family = "binomial",
  weights = weights,
...
model_baseline <- do.call(inla, c(list(formula = formula_baseline), fit_args))
model_tight    <- do.call(inla, c(list(formula = formula_tight),    fit_args))
model_loose    <- do.call(inla, c(list(formula = formula_loose),    fit_args))

# ---- 5. Extract fixed effects across all three prior specifications ----
extract_fixed <- function(model, label) {
  model$summary.fixed |>
    rownames_to_column("term") |>
    mutate(prior_spec = label) |>
    select(prior_spec, term, mean, sd, `0.025quant`, `0.5quant`, `0.975quant`)
}

sensitivity_table <- bind_rows(
  extract_fixed(model_baseline, "Baseline Priors"),
  extract_fixed(model_tight, "Priors Set 1"),
  extract_fixed(model_loose, "Priors Set 2")
)

write.csv(sensitivity_table, "prior_sensitivity_fixed_effects.csv", row.names = FALSE)

# ---- 6. Compare WAIC / DIC across prior specifications ----
fit_comparison <- tibble(
  prior_spec = c("Baseline", "Priors Set 1", "Priors Set 2"),
  waic = c(model_baseline$waic$waic, model_tight$waic$waic, model_loose$waic$waic),
  dic  = c(model_baseline$dic$dic,   model_tight$dic$dic,   model_loose$dic$dic)
)

write.csv(fit_comparison, "prior_sensitivity_fit_comparison.csv", row.names = FALSE)

# ---- 7. Coefficient comparison plot across prior specifications ----
sensitivity_plot <- ggplot(
  sensitivity_table,
  aes(x = term, y = mean, ymin = `0.025quant`, ymax = `0.975quant`, color = prior_spec)
) +
  geom_pointrange(position = position_dodge(width = 0.5)) +
  coord_flip() +
  labs(
    y = "Posterior mean (log-odds) with 95% CI",
    x = NULL,
    color = "Prior specification"
  ) +
  theme_minimal()

ggsave("prior_sensitivity_plot.png", sensitivity_plot, width = 8, height = 6, dpi = 300)
